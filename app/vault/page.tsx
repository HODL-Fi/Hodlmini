...
function VaultPageInner() {
  ...
  const [debtUsd, setDebtUsd] = React.useState<number>(Number.isFinite(debtParam) ? debtParam : 0);
        ...
        <div className="sticky top-0 z-20 bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/60">
          <div className="flex items-center justify-between py-2">
            <div className="flex items-center gap-3">
              <VaultIcon size={28} color="#111" />
              <div>
                <div className="text-[20px] font-semibold leading-6">My Vault</div>
                <div className="text-[12px] text-gray-600">Collateral value ${formatNumber(totals.totalValueUSD)}</div>
              </div>
            </div>
            <div className="inline-flex gap-2">
              <button className="rounded-full bg-gray-100 px-3 py-1.5 text-[12px] font-medium cursor-pointer">Deposit</button>
              <button className="rounded-full bg-gray-100 px-3 py-1.5 text-[12px] font-medium cursor-pointer">Withdraw</button>
            </div>
          </div>
        </div>
